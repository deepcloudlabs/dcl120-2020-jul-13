...
struct Polygon { // interface segregation principle
...
int main() {
    std::vector<std::shared_ptr<Rectangle>> rectangles{
            std::make_shared<Rectangle>(1, 1, 10, 10),
            std::make_shared<Rectangle>(2, 2, 20, 20)
    };
    for (auto &rec : rectangles) {
        for (auto &line : *rec) {
            // memory consumption -> object caching/pooling -> Flyweight Pattern
            LineToPointAdapter lpa{line}; // stateless
