...
struct tiff_reader : public image_reader, reader_chain {
    tiff_reader(image_reader *next_reader = nullptr) : reader_chain(next_reader) {}

    image *load(const std::string &file_name) {
        if (hasEnding(file_name, "tiff")) {
            std::cout << "tiff_reader loads the image" << std::endl;
            return new image;
        }
        return next(file_name);
    }
...
int main() {
    jpeg_reader reader(new gif_reader(new png_reader(new tiff_reader)));
    try {
